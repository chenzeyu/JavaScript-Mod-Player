...
function Player(name) {
    ...
    for (var i = 0; i < modfile.OrderListLength; i++) {
        ...
        for (var j = 0; j < 64; j++) {
            var currentChannel1 = currentPattern[j][0];
            var currentChannel2 = currentPattern[j][1];
            var currentChannel3 = currentPattern[j][2];
            var currentChannel4 = currentPattern[j][3];
	    ...
	    //at m_time+i*7.68, i is the number of patterns played before.
            if (currentChannel1.sample != 0) {
                if (j != 0) {
                    source1.noteOff(m_time + i * 7.68 + j * 0.12);
                }
                source1 = context.createBufferSource();
                schedSound(source1, m_time + i * 7.68 + j * 0.12, currentChannel1.pitch, ticksperSample, currentChannel1.volume, currentChannel1.volumechange, currentChannel1.sample - 1, modfile);
            }
            if (currentChannel2.sample != 0) {
                if (j != 0) {
                    source2.noteOff(m_time + i * 7.68 + j * 0.12);
                }
                source2 = context.createBufferSource();
                schedSound(source2, m_time + i * 7.68 + j * 0.12, currentChannel2.pitch, ticksperSample, currentChannel2.volume, currentChannel2.volumechange, currentChannel2.sample - 1, modfile);

            }
            if (currentChannel3.sample != 0) {
                if (j != 0) {
                    source3.noteOff(m_time + i * 7.68 + j * 0.12);
                }
                source3 = context.createBufferSource();
                schedSound(source3, m_time + i * 7.68 + j * 0.12, currentChannel3.pitch, ticksperSample, currentChannel3.volume, currentChannel3.volumechange, currentChannel3.sample - 1, modfile);

            }
            if (currentChannel4.sample != 0) {
                if (j != 0) {
                    source4.noteOff(m_time + i * 7.68 + j * 0.12);
                }
                source4 = context.createBufferSource();
                schedSound(source4, m_time + i * 7.68 + j * 0.12, currentChannel4.pitch, ticksperSample, currentChannel4.volume, currentChannel4.volumechange, currentChannel4.sample - 1, modfile);
